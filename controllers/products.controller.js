...
const getAllProducts = async (req, res) => {
  try {
    ...
    if (req.query.sortBy) {
      ...
      const sortField =
        req.query.sortBy === "price" ? "pricing.basePrice" : req.query.sortBy;
...
const getSellerProducts = async (req, res) => {
  try {
    ...
    if (!sellerId) {
      return res
        .status(400)
        .send({ success: false, message: "Seller ID is required!" });
    }
...
const updateProduct = async (req, res) => {
  try {
    ...
    if (updateData.name) {
      finalUpdateData.slug =
        updateData.name.toLowerCase().replace(/ /g, "-") + "-" + Date.now();
    }
    ...
    if (result.modifiedCount > 0) {
      ...
      res
        .status(400)
        .send({ success: false, message: "No changes made to the product." });
    }
  } catch (error) {
...
const deleteProduct = async (req, res) => {
  try {
    ...
    if (result.modifiedCount > 0) {
      ...
      res
        .status(400)
        .send({ success: false, message: "Failed to delete the product." });
    }
  } catch (error) {
...
const updateProductAnalytics = async (req, res) => {
  try {
    ...
    if (type === "view") {
      ...
      return res
        .status(400)
        .send({ success: false, message: "Invalid analytics type!" });
    }
